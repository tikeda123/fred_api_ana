-- Cross Asset Schema
-- md: マスタ, ops: 監査, fact: 正規化データ, mart: UI 用派生テーブル
...
CREATE TABLE IF NOT EXISTS ops.fact_upload_audit (
    ...
    source_file_sha256     TEXT NOT NULL,     -- 重複投入検知
...
CREATE TABLE IF NOT EXISTS fact.market_bars_raw (
    ...
    volume           DOUBLE,
    source_line_hash TEXT NOT NULL,           -- md5(row) 重複検知
    ingest_status    TEXT DEFAULT 'loaded',   -- 'loaded'|'rejected'
    quality_flags    TEXT,                    -- JSON 配列: ['high_lt_low', ...]
...
CREATE TABLE IF NOT EXISTS fact.market_bars_norm (
    ...
    session_bucket          TEXT,             -- 'asia'|'europe_open'|'us_open'|'us_late'|'other'
...
CREATE TABLE IF NOT EXISTS fact.market_bars_daily (
    ...
    timeframe_source        TEXT NOT NULL,    -- '240'
    open                    DOUBLE NOT NULL,
    high                    DOUBLE NOT NULL,
    low                     DOUBLE NOT NULL,
    close                   DOUBLE NOT NULL,
...
CREATE TABLE IF NOT EXISTS fact.cross_asset_feature_daily (
    ...
    feature_name         TEXT NOT NULL,   -- 'mom_5d'|'rv_20d'
